#[cfg(test)]
mod complex_tests {
    use keyflux_curl::{HttpMethod, parse_curl_command};
    ...
    fn parse_curl_with_query_params_should_work() {
        ...
        assert_eq!(
            query_params.get("param1").unwrap(),
            &vec!["value1".to_string()]
        );
        assert_eq!(
            query_params.get("param2").unwrap(),
    ...
    fn parse_curl_with_encoded_query_params_should_work() {
        ...
        assert_eq!(
            request.url,
            "http://example.com?param1=value%201&param2=value%202"
        );
