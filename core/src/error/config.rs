...
impl From<serde_json::Error> for ConfigError {
    ...
    fn from(error: serde_json::Error) -> Self {
        ConfigError::ParseJsonError(t!(
            "errors.config.parse_json_error",
...
impl From<toml::de::Error> for ConfigError {
    ...
    fn from(error: toml::de::Error) -> Self {
        ConfigError::ParseTomlError(t!(
            "errors.config.parse_toml_error",
            error = error.to_string()
        ).to_string())
    }
}
...
impl From<std::env::VarError> for ConfigError {
    ...
    fn from(error: std::env::VarError) -> Self {
        ConfigError::EnvVarError(t!("errors.config.env_var_error", error = error.to_string()).to_string())
    }
}
