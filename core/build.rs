...
fn main() {
    ...
    println!(
        "cargo:rustc-env=LOCALES_PATH={}/locales.rs",
    ...
    let locale_enum = locales
        .iter()
        .map(|locale| format!("    {},", locale.to_uppercase()))
        .collect::<Vec<_>>()
        .join("\n");

    let locale_impl = locales
        .iter()
        .map(|locale| {
            format!(
                "            \"{}\" => Ok(Locale::{}),",
    ...
    let locale_display_impl = locales
        .iter()
        .map(|locale| {
            format!(
                "            Locale::{} => \"{}\",",
    ...
    let supported_locales = locales
        .iter()
        .map(|locale| format!("Locale::{}", locale.to_uppercase()))
        .collect::<Vec<_>>()
        .join(", ");

    let locale_code = format!(
        r#"
...
"#,
        locale_enum = locale_enum,
        locale_impl = locale_impl,
        locale_display_impl = locale_display_impl,
