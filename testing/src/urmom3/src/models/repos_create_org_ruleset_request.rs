/*
 * GitHub v3 REST API
 *
 * GitHub's v3 REST API.
 *
 * The version of the OpenAPI document: 1.1.4
 * 
 * Generated by: https://openapi-generator.tech
 */




#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct ReposCreateOrgRulesetRequest {
    /// The name of the ruleset.
    #[serde(rename = "name")]
    pub name: String,
    /// The target of the ruleset  **Note**: The `push` target is in beta and is subject to change.
    #[serde(rename = "target", skip_serializing_if = "Option::is_none")]
    pub target: Option<Target>,
    #[serde(rename = "enforcement")]
    pub enforcement: crate::models::RepositoryRuleEnforcement,
    /// The actors that can bypass the rules in this ruleset
    #[serde(rename = "bypass_actors", skip_serializing_if = "Option::is_none")]
    pub bypass_actors: Option<Vec<crate::models::RepositoryRulesetBypassActor>>,
    #[serde(rename = "conditions", skip_serializing_if = "Option::is_none")]
    pub conditions: Option<Box<crate::models::OrgRulesetConditions>>,
    /// An array of rules within the ruleset.
    #[serde(rename = "rules", skip_serializing_if = "Option::is_none")]
    pub rules: Option<Vec<crate::models::RepositoryRule>>,
}

impl ReposCreateOrgRulesetRequest {
    pub fn new(name: String, enforcement: crate::models::RepositoryRuleEnforcement) -> ReposCreateOrgRulesetRequest {
        ReposCreateOrgRulesetRequest {
            name,
            target: None,
            enforcement,
            bypass_actors: None,
            conditions: None,
            rules: None,
        }
    }
}

/// The target of the ruleset  **Note**: The `push` target is in beta and is subject to change.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Target {
    #[serde(rename = "branch")]
    Branch,
    #[serde(rename = "tag")]
    Tag,
    #[serde(rename = "push")]
    Push,
}

impl Default for Target {
    fn default() -> Target {
        Self::Branch
    }
}

