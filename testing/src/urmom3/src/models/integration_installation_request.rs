/*
 * GitHub v3 REST API
 *
 * GitHub's v3 REST API.
 *
 * The version of the OpenAPI document: 1.1.4
 * 
 * Generated by: https://openapi-generator.tech
 */

/// IntegrationInstallationRequest : Request to install an integration on a target



#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct IntegrationInstallationRequest {
    /// Unique identifier of the request installation.
    #[serde(rename = "id")]
    pub id: i32,
    #[serde(rename = "node_id", skip_serializing_if = "Option::is_none")]
    pub node_id: Option<String>,
    #[serde(rename = "account")]
    pub account: Box<crate::models::IntegrationInstallationRequestAccount>,
    #[serde(rename = "requester")]
    pub requester: Box<crate::models::SimpleUser>,
    #[serde(rename = "created_at")]
    pub created_at: String,
}

impl IntegrationInstallationRequest {
    /// Request to install an integration on a target
    pub fn new(id: i32, account: crate::models::IntegrationInstallationRequestAccount, requester: crate::models::SimpleUser, created_at: String) -> IntegrationInstallationRequest {
        IntegrationInstallationRequest {
            id,
            node_id: None,
            account: Box::new(account),
            requester: Box::new(requester),
            created_at,
        }
    }
}


