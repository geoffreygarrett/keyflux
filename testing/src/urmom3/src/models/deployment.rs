/*
 * GitHub v3 REST API
 *
 * GitHub's v3 REST API.
 *
 * The version of the OpenAPI document: 1.1.4
 * 
 * Generated by: https://openapi-generator.tech
 */

/// Deployment : The [deployment](https://docs.github.com/rest/deployments/deployments#list-deployments).



#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct Deployment {
    #[serde(rename = "created_at")]
    pub created_at: String,
    #[serde(rename = "creator")]
    pub creator: Option<Box<crate::models::User>>,
    #[serde(rename = "description")]
    pub description: Option<String>,
    #[serde(rename = "environment")]
    pub environment: String,
    #[serde(rename = "id")]
    pub id: i32,
    #[serde(rename = "node_id")]
    pub node_id: String,
    #[serde(rename = "original_environment")]
    pub original_environment: String,
    #[serde(rename = "payload")]
    pub payload: Box<crate::models::DeploymentPayload>,
    #[serde(rename = "performed_via_github_app", skip_serializing_if = "Option::is_none")]
    pub performed_via_github_app: Option<Box<crate::models::App5>>,
    #[serde(rename = "production_environment", skip_serializing_if = "Option::is_none")]
    pub production_environment: Option<bool>,
    #[serde(rename = "ref")]
    pub r#ref: String,
    #[serde(rename = "repository_url")]
    pub repository_url: String,
    #[serde(rename = "sha")]
    pub sha: String,
    #[serde(rename = "statuses_url")]
    pub statuses_url: String,
    #[serde(rename = "task")]
    pub task: String,
    #[serde(rename = "transient_environment", skip_serializing_if = "Option::is_none")]
    pub transient_environment: Option<bool>,
    #[serde(rename = "updated_at")]
    pub updated_at: String,
    #[serde(rename = "url")]
    pub url: String,
}

impl Deployment {
    /// The [deployment](https://docs.github.com/rest/deployments/deployments#list-deployments).
    pub fn new(created_at: String, creator: Option<crate::models::User>, description: Option<String>, environment: String, id: i32, node_id: String, original_environment: String, payload: crate::models::DeploymentPayload, r#ref: String, repository_url: String, sha: String, statuses_url: String, task: String, updated_at: String, url: String) -> Deployment {
        Deployment {
            created_at,
            creator: if let Some(x) = creator {Some(Box::new(x))} else {None},
            description,
            environment,
            id,
            node_id,
            original_environment,
            payload: Box::new(payload),
            performed_via_github_app: None,
            production_environment: None,
            r#ref,
            repository_url,
            sha,
            statuses_url,
            task,
            transient_environment: None,
            updated_at,
            url,
        }
    }
}


