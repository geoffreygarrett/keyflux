  ...
  class CodeSecurityCreateConfigurationRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CodeSecurityCreateConfigurationRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CodeSecurityCreateConfigurationRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'advanced_security')
        ...
        self.advanced_security = 'disabled'
      end
      ...
      if attributes.key?(:'private_vulnerability_reporting')
        ...
        self.private_vulnerability_reporting = 'disabled'
      end
    end
    ...
    def hash
      [name, description, advanced_security, dependency_graph, dependabot_alerts, dependabot_security_updates, code_scanning_default_setup, secret_scanning, secret_scanning_push_protection, private_vulnerability_reporting].hash
    end
