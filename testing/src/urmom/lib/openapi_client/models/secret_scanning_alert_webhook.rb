  ...
  class SecretScanningAlertWebhook
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SecretScanningAlertWebhook` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SecretScanningAlertWebhook`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [number, created_at, updated_at, url, html_url, locations_url, resolution, resolved_at, resolved_by, resolution_comment, secret_type, secret_type_display_name, validity, push_protection_bypassed, push_protection_bypassed_by, push_protection_bypassed_at].hash
    end
