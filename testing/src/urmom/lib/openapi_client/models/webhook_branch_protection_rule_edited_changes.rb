  ...
  class WebhookBranchProtectionRuleEditedChanges
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::WebhookBranchProtectionRuleEditedChanges` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::WebhookBranchProtectionRuleEditedChanges`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'authorized_actor_names')
        self.authorized_actor_names = attributes[:'authorized_actor_names']
      end
    ...
    def hash
      [admin_enforced, authorized_actor_names, authorized_actors_only, authorized_dismissal_actors_only, linear_history_requirement_enforcement_level, lock_branch_enforcement_level, lock_allows_fork_sync, pull_request_reviews_enforcement_level, require_last_push_approval, required_status_checks, required_status_checks_enforcement_level].hash
    end
