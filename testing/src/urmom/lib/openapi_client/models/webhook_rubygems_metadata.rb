  ...
  class WebhookRubygemsMetadata
    ...
    def self.openapi_types
      {
        ...
        :'repo' => :'String',
        :'dependencies' => :'Array<Hash<String, String>>',
        :'commit_oid' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::WebhookRubygemsMetadata` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::WebhookRubygemsMetadata`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [name, description, readme, homepage, version_info, platform, metadata, repo, dependencies, commit_oid].hash
    end
