  ...
  class Issue14
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Issue14` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Issue14`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      locked_validator = EnumAttributeValidator.new('Boolean', ["false"])
      return false unless locked_validator.valid?(@locked)
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] locked Object to be assigned
    def locked=(locked)
      validator = EnumAttributeValidator.new('Boolean', ["false"])
      unless validator.valid?(locked)
        fail ArgumentError, "invalid value for \"locked\", must be one of #{validator.allowable_values}."
      end
      @locked = locked
    end
