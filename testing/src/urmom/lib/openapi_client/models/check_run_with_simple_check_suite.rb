  ...
  class CheckRunWithSimpleCheckSuite
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CheckRunWithSimpleCheckSuite` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CheckRunWithSimpleCheckSuite`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @check_suite.nil?
        invalid_properties.push('invalid value for "check_suite", check_suite cannot be nil.')
      end

      if @details_url.nil?
        invalid_properties.push('invalid value for "details_url", details_url cannot be nil.')
      end
    ...
    def valid?
      return false if @check_suite.nil?
      conclusion_validator = EnumAttributeValidator.new('String', ["waiting", "pending", "startup_failure", "stale", "success", "failure", "neutral", "cancelled", "skipped", "timed_out", "action_required"])
      return false unless conclusion_validator.valid?(@conclusion)
      return false if @details_url.nil?
      ...
      status_validator = EnumAttributeValidator.new('String', ["queued", "in_progress", "completed", "pending"])
    ...
    def conclusion=(conclusion)
      validator = EnumAttributeValidator.new('String', ["waiting", "pending", "startup_failure", "stale", "success", "failure", "neutral", "cancelled", "skipped", "timed_out", "action_required"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["queued", "in_progress", "completed", "pending"])
    ...
    def hash
      [app, check_suite, completed_at, conclusion, deployment, details_url, external_id, head_sha, html_url, id, name, node_id, output, pull_requests, started_at, status, url].hash
    end
