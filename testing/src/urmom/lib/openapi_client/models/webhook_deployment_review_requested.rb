  ...
  class WebhookDeploymentReviewRequested
    ...
    def self.openapi_types
      {
        ...
        :'workflow_run' => :'DeploymentWorkflowRun3'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::WebhookDeploymentReviewRequested` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::WebhookDeploymentReviewRequested`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @workflow_job_run.nil?
        invalid_properties.push('invalid value for "workflow_job_run", workflow_job_run cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @workflow_job_run.nil?
      true
    end
    ...
    def hash
      [action, enterprise, environment, installation, organization, repository, requestor, reviewers, sender, since, workflow_job_run, workflow_run].hash
    end
