...
module OpenapiClient
  class WebhookPackageUpdatedPackagePackageVersion
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::WebhookPackageUpdatedPackagePackageVersion` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::WebhookPackageUpdatedPackagePackageVersion`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [author, body, body_html, created_at, description, docker_metadata, draft, html_url, id, installation_command, manifest, metadata, name, package_files, package_url, prerelease, release, rubygems_metadata, source_url, summary, tag_name, target_commitish, target_oid, updated_at, version].hash
    end
