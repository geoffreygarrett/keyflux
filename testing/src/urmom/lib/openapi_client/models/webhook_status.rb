  ...
  class WebhookStatus
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::WebhookStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::WebhookStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      state_validator = EnumAttributeValidator.new('String', ["pending", "success", "failure", "error"])
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["pending", "success", "failure", "error"])
    ...
    def hash
      [avatar_url, branches, commit, context, created_at, description, enterprise, id, installation, name, organization, repository, sender, sha, state, target_url, updated_at].hash
    end
