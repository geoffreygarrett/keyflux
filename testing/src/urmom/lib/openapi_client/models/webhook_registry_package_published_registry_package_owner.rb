...
module OpenapiClient
  class WebhookRegistryPackagePublishedRegistryPackageOwner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::WebhookRegistryPackagePublishedRegistryPackageOwner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::WebhookRegistryPackagePublishedRegistryPackageOwner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @gravatar_id.nil?
        invalid_properties.push('invalid value for "gravatar_id", gravatar_id cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @gravatar_id.nil?
    ...
    def hash
      [avatar_url, events_url, followers_url, following_url, gists_url, gravatar_id, html_url, id, login, node_id, organizations_url, received_events_url, repos_url, site_admin, starred_url, subscriptions_url, type, url].hash
    end
