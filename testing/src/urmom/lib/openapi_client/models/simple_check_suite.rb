  ...
  class SimpleCheckSuite
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SimpleCheckSuite` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SimpleCheckSuite`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      conclusion_validator = EnumAttributeValidator.new('String', ["success", "failure", "neutral", "cancelled", "skipped", "timed_out", "action_required", "stale", "startup_failure"])
      return false unless conclusion_validator.valid?(@conclusion)
      status_validator = EnumAttributeValidator.new('String', ["queued", "in_progress", "completed", "pending", "waiting"])
    ...
    def conclusion=(conclusion)
      validator = EnumAttributeValidator.new('String', ["success", "failure", "neutral", "cancelled", "skipped", "timed_out", "action_required", "stale", "startup_failure"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["queued", "in_progress", "completed", "pending", "waiting"])
    ...
    def hash
      [after, app, before, conclusion, created_at, head_branch, head_sha, id, node_id, pull_requests, repository, status, updated_at, url].hash
    end
