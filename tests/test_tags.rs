...
mod tests {
    ...
    use keyflux::config::tags::{Enum, include_file, process_custom_tag, process_mapping};
    use log::info;
    use std::sync::Once;
    use std::fs;
    ...
    fn test_include_file() {
        ...
        assert_eq!(result.unwrap(), serde_yaml::from_str::<Value>(&read_file_content("test.yaml")).unwrap());
    }
    ...
    fn test_include_file_recursion_depth_exceeded() {
        ...
        assert!(result.is_err(), "Result was OK when it should have been an error");
    ...
    fn test_include_file_and_process_mapping_integration() {
        ...
        assert!(file_path.exists(), "File does not exist: {:?}", file_path);
        assert!(subfile1_path.exists(), "File does not exist: {:?}", subfile1_path);
        assert!(subfile2_path.exists(), "File does not exist: {:?}", subfile2_path);
        assert!(subfile3_path.exists(), "File does not exist: {:?}", subfile3_path);

        info!("Testing include_file_and_process_mapping_integration with path: {:?}", file_path);
        let result = include_file(file_path.to_str().unwrap(), &current_path, 0);
        assert!(result.is_ok(), "Result was not OK: {:?}", result);
...
"#;
    ...
    fn test_process_mapping_with_no_includes() {
        ...
        map.insert(Value::String("key1".to_string()), Value::String("value1".to_string()));
        map.insert(Value::String("key2".to_string()), Value::String("value2".to_string()));
        let current_dir = std::env::current_dir().unwrap();
        let current_path = PathBuf::from(current_dir.to_str().unwrap());
    ...
    fn test_process_mapping_with_multiple_includes() {
        ...
        info!("Testing process_mapping_with_multiple_includes with path: {:?}", file_path);
        let result = include_file(file_path.to_str().unwrap(), &current_path, 0);
        assert!(result.is_ok(), "Result was not OK: {:?}", result);
