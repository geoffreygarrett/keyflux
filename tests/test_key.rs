...
#[cfg(test)]
mod tests {
    use keyflux::key::{Attributes, AttributeValue, Key};
    ...
    fn test_attributes_insert_and_get_multiple() {
        let mut attributes = Attributes::new();
        attributes.insert("key1", AttributeValue::Multiple(vec!["value1".to_string(), "value2".to_string()]));
        assert_eq!(attributes.get("key1"), Some(&AttributeValue::Multiple(vec!["value1".to_string(), "value2".to_string()])));
    }
    ...
    fn test_attributes_remove_multiple() {
        let mut attributes = Attributes::new();
        attributes.insert("key1", AttributeValue::Multiple(vec!["value1".to_string(), "value2".to_string()]));
        assert_eq!(attributes.remove("key1"), Some(AttributeValue::Multiple(vec!["value1".to_string(), "value2".to_string()])));
        assert!(attributes.get("key1").is_none());
    }
    ...
    fn test_attributes_serialize_deserialize() {
        let mut attributes = Attributes::new();
        attributes.insert("key1", AttributeValue::Single("value1".to_string()));
        attributes.insert("key2", AttributeValue::Multiple(vec!["value2".to_string(), "value3".to_string()]));
    ...
    fn test_key_with_attributes() {
        ...
        assert_eq!(key.attributes.get("attr1"), Some(&AttributeValue::Single("value1".to_string())));
    }
